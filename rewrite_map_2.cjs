...
function AssamMap({ reports }) {
  ...
  return (
    <div className="card" style={{ padding: 24, background: "var(--bg-main)" }}>
      ...
      <div ref={containerRef} style={{ width: "100%", aspectRatio: "4/3", background: "#f8fafc", borderRadius: 16, overflow: "hidden", cursor: zoom > 1 ? (dragging ? "grabbing" : "grab") : "default", userSelect: "none", position: "relative", border: "1px solid var(--border-color)" }} onMouseDown={onMD} onMouseMove={onMM} onMouseUp={onMU} onMouseLeave={onMU}>
        ...
        <div style={{ position: "absolute", top: 16, right: 16, background: "var(--accent-primary)", color: "#fff", padding: "8px 16px", borderRadius: 20, fontWeight: 700, fontSize: 14, boxShadow: "0 4px 12px rgba(0,0,0,0.1)", zIndex: 10 }}>
          {reports.length} reports
        </div>
        ...
        <svg viewBox="0 0 100 50" style={{ width: "100%", height: "100%", transform: \`scale(\${zoom}) translate(\${pan.x/10}px,\${pan.y/10}px)\`, transformOrigin: "center center", transition: dragging ? "none" : "transform 0.1s" }} preserveAspectRatio="xMidYMid meet">
          <image href={ASSAM_MAP_SRC} width="100" height="50" opacity="0.6" preserveAspectRatio="none" style={{ filter: "sepia(1) hue-rotate(180deg) saturate(0.5) opacity(0.8)" }} />
          {gps.map((r, i) => {
            const cx = tx(parseFloat(r.lat)), cy = ty(parseFloat(r.lng)); // wait, long is X, lat is Y!
            // Wait! Previous code was: tx(r.lng), ty(r.lat)
            const w = WASTE.find(t => t.id === r.waste_type), col = w?.color || "#9CA3AF";
            return (
              <g key={\`g\${i}\`} className="pin-g" style={{ animationDelay: \`\${i * 0.05}s\` }}>
                <circle cx={tx(parseFloat(r.lng) || 92)} cy={ty(parseFloat(r.lat) || 26) - 2.8} r="2.5" fill={col} stroke="#fff" strokeWidth="0.6" opacity={0.9} style={{ filter: "drop-shadow(0 2px 2px rgba(0,0,0,0.3))" }}/>
                <line x1={tx(parseFloat(r.lng) || 92)} y1={ty(parseFloat(r.lat) || 26) - 0.6} x2={tx(parseFloat(r.lng) || 92)} y2={ty(parseFloat(r.lat) || 26)} stroke={col} strokeWidth="1" strokeLinecap="round"/>
              </g>
            );
          })}
          {nogps.map((r, i) => {
...
const mapEnd = code.indexOf('// ── Report Card ───────────────────────────────────────────────────────────');
code = code.substring(0, mapStart) + newAssamMap + code.substring(mapEnd);

// Strip MapResetControl which was previously inserted into PieChart!
// Wait! Earlier I did: code.replace('function PieChart({ reports }) {', mapResetComp + '\\nfunction PieChart({ reports }) {');
const mrcStart = code.indexOf('function MapResetControl() {');
const mrcEnd = code.indexOf('function PieChart({ reports }) {');
if (mrcStart > -1 && mrcEnd > -1) {
  code = code.substring(0, mrcStart) + code.substring(mrcEnd);
}
